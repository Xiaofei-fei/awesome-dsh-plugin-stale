...
- [Xiaofei-fei/dsh-prompt-history](https://github.com/Xiaofei-fei/dsh-prompt-history) — 为 DSH Web 输入框提供终端式复制粘贴与类 Linux shell 的提示词历史：选中工具栏支持复制与引用（引用将完整内容以简约的 > 引用块直接插入输入框）、右键直接粘贴、↑/↓ 召回之前发送过的消息（支持前缀搜索与 Ctrl+R 反向搜索），设置面板可切换复制模式；历史取自会话自身的消息记录（零配置、零额外存储）。
